...
const EventBlock = <T extends GenericEvent>({
	...
	return (
		<div
			...
			key={index}
		>
			<p style={{ color: 'white', fontSize: '12px', paddingLeft: '5px' }}>{event.title}</p>
		</div>
	);
};
...
function Calendar<T extends GenericEvent>({
	...
	const dayColumns = dayList.map((day, counter) => {
		...
		return {
			...
			key: day,
			width: 2,
			render: function (events: ColumnNode<T>, row: EventsObject<T>): React.ReactNode | undefined {
				if (events && events.length > 0 && events instanceof Array) {
					const eventsBlock = events.map(function (event, index: number): React.ReactNode {
						return (
							<EventBlock
								key={event.eventId}
								event={event}
	...
	const hourColumn = {
		...
		key: 'hour',
		width: 1,
		render: (hour: React.ReactNode, {}, id: number) => {
			return {
				props: {
					style: { width: '10%' },
				},
				children: SCROLL_TO_ROW === id ? <div ref={rowRef}>{hour}</div> : <div>{hour}</div>,
			};
		},
	};
