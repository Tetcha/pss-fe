...
import { WeekObject, EventsObject, GetWeekDates, GenericEvent, DayName } from './types';

export const daysToWeekObject = <T extends GenericEvent>(events: T[], startWeek: Date) => {
	...
	for (const googleEventIndex in events) {
		...
		if (!isSameDay(eventStartTimeDay, eventEndTimeDay)) {
			...
			for (const dayInterval in result) {
				...
				const weekObjectKey: DayName = dayNames[getDay(new Date(result[dayInterval]))];
		...
		events.push({
			...
			Wednesday:
				...
				weekObject?.wednesday.filter((e) => {
					return e.allDay ? i === ALL_DAY_EVENT : isSameHour(e.startTime, add(hour, { days: 2 }));
				}),
			Thursday:
				weekObject?.thursday &&
				weekObject?.thursday.filter((e) => {
					return e.allDay ? i === ALL_DAY_EVENT : isSameHour(e.startTime, add(hour, { days: 3 }));
				}),
			Friday:
				weekObject?.friday &&
				weekObject?.friday.filter((e) => {
					return e.allDay ? i === ALL_DAY_EVENT : isSameHour(e.startTime, add(hour, { days: 4 }));
				}),
			Saturday:
				weekObject?.saturday &&
				weekObject?.saturday.filter((e) => {
					return e.allDay ? i === ALL_DAY_EVENT : isSameHour(e.startTime, add(hour, { days: 5 }));
				}),
			Sunday:
				weekObject?.sunday &&
				weekObject?.sunday.filter((e) => {
					return e.allDay ? i === ALL_DAY_EVENT : isSameHour(e.startTime, add(hour, { days: 6 }));
				}),
		});
	}
